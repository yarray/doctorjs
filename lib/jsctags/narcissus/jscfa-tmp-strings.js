...
// merge all string properties of the object to one generic string property
Aobj.prototype.mergeStringProps = function() {
  if (this.stringPropsMerged) return;
  var av = BOTTOM, self = this;
  this.forEachOwnProp(function(p) {
      if (propIsString(p)) {
        av = avjoin(av, self[p]);
  ...
  this.updateProp("-string", av); // this bumps timestamp, don't bump again
  this.stringPropsMerged = true;
};
...
function addSummary(n, args, retval, errval) {
  //print("adding summary for " + n.fname.name + n.lineno + "\n");
...
(function() {
  ...
  function _index(n, ans, oldlval) {
    ...
    //   avobj.updateProp(prop.value + "-", rval);
      ...
      avobj.forEachObj(function(o) {
          if (o.stringPropsMerged)
            o.updateProp("-string", rval);
          else
            o.updateProp(prop.value + "-", rval);
        });
    }
      ...
      if (avprop.hasStr()) {
        avobj.forEachObj(function(o) { o.mergeStringProps(); });
  ...
  function _dot(n, ans, oldlval) {
    ...
    ans2.v.forEachObj(function(o) {
        if (o.stringPropsMerged)
          o.updateProp("-string", ans.v);
        else
          o.updateProp(n[1].name, ans.v);
      });
    ans.fr = ans2.fr;
...
(function() {
  ...
  function _index(n, fr) {
    ...
    if (pt === NUMBER) {
      ...
      avobj.forEachObj(function(o) {
          if (o.stringPropsMerged)
            av = avjoin(av, o.getOwnProp("-string"));
          else
            av = avjoin(av, o.getProp(prop.value + "-") || AUNDEF);
        });
    }
    else {
  ...
  function _dot(n, fr) {
    var ans = evalExp[n[0].type](n[0], fr), av = BOTTOM;
    ans.thisav = ans.v; // used by method calls
    ans.v.forEachMayObj(function(o) {
        if (o.stringPropsMerged)
          av = avjoin(av, o.getOwnProp("-string"));
        else
          av = avjoin(av, o.getProp(n[1].name) || AUNDEF);
      });
    ans.v = av;
    return ans;
  }
...
function evalFun(fn, args, withNew) {
  ...
  //print("entered fun: "+ fn.fname.name + fn.lineno +"\n");
  ...
  while(true) {
    try {
      ...
      if (!p2ts) {
        ...
        if (pelm2[3]) {
          //print("recursion bottoms\n");
          --depth;
          return new Ans(BOTTOM, undefined);
        }
        else {
          //print("recursion continues\n");
...
function cfa2(ast) {
  ...
  try {
    ...
    //throw e;
    //print(e.hasOwnProperty("stack") + " " + (e instanceof Error) + " \n");
    print(e.message);
  }
}
