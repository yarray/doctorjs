...
Aobj.prototype.toType = function(seenObjs) {
  ...
  var c = this.getExactProp("constructor-");
  var types = [];
...
// string -> Aval or undefined
// Looks for the exact property called pname
Aobj.prototype.getExactProp = function(pname) {
  return getProp(this, pname, false);
};
...
function getProp(o, pname, lax) {
  function act(o) {
    ...
    var v = {val : undefined, stop : false};
    if (lax) {
      if (o.numPropsMerged && propIsNumeric(pname))
        v.val = o["-number"].aval;
      if (o.strPropsMerged)
        v.val = maybeavjoin(v.val, o["-string"].aval);
    }
    return v;
  }
...
Aobj.prototype.getNumProps = function() {
  function act(o) {
    o.mergeNumProps();
    return {val : o["-number"].aval, stop : false};
  }
  function combine(avs) {
    var val = BOTTOM;
...
Aobj.prototype.updateProp = function(pname, newv) {
  ...
  if (this.hasOwnProperty(pname))
    ...
    if (!this.numPropsMerged) { // strings aren't merged either
      this[pname] = new Aprop({aval : newv});
      ++timestamp;
    }
    else {
      if (propIsNumeric(pname)) upd("-number", this["-number"], newv);
      if (this.strPropsMerged) upd("-string", this["-string"], newv);
    }
  }
};
...
Aobj.prototype.forEachOwnProp = function(f) {
  for (var p in this)
    if (this[p] instanceof Aprop) 
      f(p, this[p].aval); // f may ignore its second argument
};

Aobj.prototype.forEachProp = function(f) {
  forEachProp(this, f, false, [], []);
};

Aobj.prototype.forEachEnumProp = function(f) {
  forEachProp(this, f, true, [], []);
};

// Uses seenObjs to avoid looping on cycles in the prototype chain.
// Uses seenProps to avoid considering shadowed properties.
function forEachProp(o, f, enumsOnly, seenObjs, seenProps) {
  for (var p in o) {
    var pobj = o[p];
    if ((pobj instanceof Aprop) 
        && (enumsOnly ? pobj.enum : true)
        && !seenProps.member(function(pname) { return p === pname; })) {
      f(p, pobj.aval); // f may ignore its second argument
      seenProps.push(p);
    }
  }
  if (!o[0]) return; // reached the end of the prototype chain
  if (seenObjs.member(function(o2) { return o === o2; }))
    return;
  else
    seenObjs.push(o);
  var solen = seenObjs.length, splen = seenProps.length;
  o[0].forEachObj(function(proto) {
      forEachProp(proto, f, enumsOnly, seenObjs, seenProps);
      seenObjs.splice(solen, seenObjs.length);
      seenProps.splice(splen, seenProps.length);
    });
}
...
Aval.prototype.forEachMayObj = function(fun) {
  ...
  if ((base & 7) !== 0) { // need to convert a base value to an object
...
// string -> Aval
Aval.prototype.getExactProp = function(prop) {
  var av = BOTTOM, av2;
  this.forEachMayObj(function(o) {
      av2 = o.getExactProp(prop);
...
// string, Aval -> undefined
Aval.prototype.updateExactProp = function(prop, av) {
  // FIXME: record error if avobjs contains base values
  this.forEachObj(function(o) { o.updateExactProp(prop, av); });
};
...
function initCoreObjs() {
  ...
  function attachMethod(o, mname, mcode, arity) {
    ...
    o.addProp(mname + "-", 
              {aval : makeObjAval(foaddr), enum : false});
  }
  ...
  heap[++count] = AUNDEF;
  go.addProp("Infinity-", 
             {aval : ANUM, write : false, enum : false, config : false});
  go.addProp("NaN-", 
             {aval : ANUM, write : false, enum : false, config : false});
  go.addProp("undefined-", 
             {aval : AUNDEF, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    var aaddr = ++count, aav = makeObjAval(aaddr), aavaddr = ++count;
    heap[aaddr] = a;
    heap[aavaddr] = aav;
    go.addProp("Array-", {aval : aav, enum : false});
    a.addProp("prototype-", 
              {aval : apav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    var naddr = ++count, nav = makeObjAval(naddr), navaddr = ++count;
    heap[naddr] = n;
    heap[navaddr] = nav;
    go.addProp("Number-",{aval : nav, enum : false});
    n.addProp("prototype-", 
              {aval : npav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    omatch.addProp("-number", {aval : ASTR});
    ...
    osplit.addProp("-number", {aval : ASTR});
    ...
    var saddr = ++count, sav = makeObjAval(saddr), savaddr = ++count;
    heap[saddr] = s;
    heap[savaddr] = sav;
    go.addProp("String-", {aval : sav, enum : false});
    s.addProp("prototype-", 
              {aval : spav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    var eaddr = ++count, eav = makeObjAval(eaddr), eavaddr = ++count;
    heap[eaddr] = e;
    heap[eavaddr] = eav;
    go.addProp("Error-", {aval : eav, enum : false});
    e.addProp("prototype-", 
              {aval : epav, write : false, enum : false, config : false});
    ...
    se.addProp("prototype-", 
               {aval : sepav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    oexec.addProp("-number", {aval : ASTR});
    ...
    function _RegExp(args) {
      args[0].forEachObj(function (o) {
          ...
          o.addProp("multiline-", 
                    {aval : ABOOL, write : false, enum : false, config :false});
          o.addProp("source-", 
                    {aval : ASTR, write : false, enum : false, config : false});
        });
      return new Ans(args[0]);
    }
    regexp_constructor = _RegExp;
    var r = new Aobj({proto : o_pav, fun : funToNode("RegExp", _RegExp)});
    var raddr = ++count, rav = makeObjAval(raddr), ravaddr = ++count;
    heap[raddr] = r;
    heap[ravaddr] = rav;
    go.addProp("RegExp-", {aval : rav, enum : false});
    r.addProp("prototype-", 
              {aval : rpav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    var daddr = ++count, dav = makeObjAval(daddr), davaddr = ++count;
    heap[daddr] = d;
    heap[davaddr] = dav;
    go.addProp("Date-", {aval : dav, enum : false});
    d.addProp("prototype-", 
              {aval : dpav, write : false, enum : false, config : false});
  ...
  (function () {
    ...
    var maddr = ++count, mav = makeObjAval(maddr), mavaddr = ++count;
    heap[maddr] = m;
    heap[mavaddr] = mav;
  ...
  (function () {
    ...
    var baddr = ++count, bav = makeObjAval(baddr), bavaddr = ++count;
    heap[baddr] = b;
    heap[bavaddr] = bav;
    go.addProp("Boolean-", {aval : bav, enum : false});
    b.addProp("prototype-", 
              {aval : bpav, write : false, enum : false, config : false});
...
(function() {
  ...
  function _index(n, ans, oldlval) {
    ...
    if (pt === NUMBER) {
      avobj.forEachObj(function(o) {
          if (o.numPropsMerged)
            o.updateExactProp("-number", rval);
          else
            o.updateExactProp(prop.value.toString() + "-", rval);
        });
    }
    else if (pt === STRING)
      avobj.updateExactProp(prop.value, rval);
    else {
      ...
      if (avprop.hasNum()) {
        ...
        avobj.updateExactProp("-number", rval);
      }
      if (avprop.hasStr()) {
        var slit = avprop.getStrLit();
        if (slit)
          avobj.updateExactProp(slit, rval);
        else
          avobj.updateExactProp("-string", rval);
      }
    }
  ...
  function _dot(n, ans, oldlval) {
    ...
    ans2.v.updateExactProp(n[1].name, ans.v);
    ans.fr = ans2.fr;
  ...
  function _dot_proto(n, ans, oldlval) {
    ...
    var ans2 = evalExp[n[0].type](n[0], ans.fr);
    ans2.v.forEachObj(function (o) {
        if (o.getFun() && !o.getOwnExactProp("prototype-"))
          o.addProp("prototype-", {aval : ans.v, enum : false});
        else
          o.updateExactProp("prototype-", ans.v);
      });
    ans.fr = ans2.fr;
    ans.err = maybeavjoin(ans.err, ans2.err);
    return ans;
...
(function() {
  ...
  function _obj_init(n, fr) {
    ...
    n.forEach(function(pinit) {
        ...
        fr = ans.fr;
        newobj.updateExactProp(pinit[0].name, ans.v);
        errval = maybeavjoin(errval, ans.err);
      });
  ...
  function _array_init(n, fr) {
    ...
    n.forEach(function(elm, i) {
        ...
        fr = ans.fr;
        newarray.updateExactProp(i + "-", ans.v);
        errval = maybeavjoin(errval, ans.err);
      });
  ...
  function _dot_proto(n, fr) {
    ...
    ans.v.forEachObj(function(o) {
        ...
        if (!clos) { // if o isn't a function, this is just a property access
          av2 = o.getExactProp("prototype-");
          av = avjoin(av, av2 || AUNDEF);
        }
        else if (proto = o.getOwnExactProp("prototype-")) {
          av = avjoin(av, proto);
        }
        else {// create default prototype and return it
          proto = makeDefaultProto(clos);
          o.addProp("prototype-", {aval : proto, enum : false});
          av = avjoin(av, proto);
        }
      });
  ...
  function _index(n, fr) {
    ...
    // can be called from pt === STRING also and from the slit case
    // and sth similar would be done if string props are merged
    // So decide how to handle string prop merging and then do this
    function getNumeric(avobj, s) {
      var av = BOTTOM;
      avobj.forEachObj(function(o) {
          if (o.numPropsMerged)
            av = avjoin(av, o.getOwnExactProp("-number"));
          else
            av = avjoin(av, o.getExactProp(s) || AUNDEF);
        });
      return av;
    }
    ...
    if (pt === NUMBER) {
      av = getNumeric(avobj, prop.value.toString() + "-");
    }
    else if (pt === STRING)
      av = avobj.getExactProp(prop.value);
    else {
      // else merge nums to a generic number & join strings but don't merge.
      ...
      if (avprop.hasStr()) {
        var slit = avprop.getStrLit();
        if (slit)
          av = avobj.getExactProp(slit);
        else 
          avobj.forEachObj(function(o) {
              o.forEachProp(function(pname, pval) {
                  if (propIsString(pname)) av = avjoin(av, pval);
                });
            });
      }
    }
  ...
  function _dot(n, fr) {
    ...
    ans.v = ans.v.getExactProp(n[1].name);
    return ans;
  }
  ...
  function _new_with_args(n, fr) {
    ...
    ans.v.forEachMayObj(function(o) {
        var clos = o.getFun(), proto;
        if (!clos) return;
        if (!(proto = o.getOwnExactProp("prototype-"))) {
