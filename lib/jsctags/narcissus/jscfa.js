...
// string, Aval, node -> undefined
Aobj.prototype.updateProp = function(prop, newv, node) {
...
function initCoreObjs() {
  ...
  if (browserMode) {
    ...
    go.updateProp("window-", new Aval(NOBASEVALS, goaddr));
  }
...
function getTags(ast, pathtofile, lines, options) {
  ...
  // Build up a list of core objects that are blacklisted from appearing in
  // tags files.
  var coreObjects = {};
  for (var key in core) {
    var coreAval = heap[core[key]];
    coreAval.objs.forEach(function(objaddr) {
      coreObjects[objaddr] = true;
    });
  }
  ...
  function walk(aobj, path, seen) {
    aobj.forEachOwnProp(function(p) {
      if (p[0] === "-") {
        return; // This is -number or -string and is unlikely to be helpful.
      }
      ...
      av.objs.forEach(function(objaddr) {
        if (seen[objaddr] || objaddr in coreObjects)
          return;
