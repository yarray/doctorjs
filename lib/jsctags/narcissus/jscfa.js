...
function getTags(ast, pathtofile, lines, options) {
  ...
  function addTagsForObj(o, propLines) {
    o.forEachOwnProp(function (p) {
        var av = o.getOwnProp(p);
        var tag = {};
  ...
  if (exportsObj.obj) {
    addTagsForObj(exportsObj.obj, exportsObj.lines);
  }
  if (browserMode) {
    var globalObjectAddr = heap[core["global object"]].objs[0];
    sys.debug(globalObjectAddr);
    sys.debug(sys.inspect(heap[globalObjectAddr]));
    addTagsForObj(heap[globalObjectAddr], null);
  }
