...
Aobj.prototype.hasProp = function(pred) {
  return hasProp(this, pred, []);
};

// Uses seenObjs to avoid looping on cycles in the prototype chain.
function hasProp(o, pred, seenObjs) {
  for (var p in o) { if (pred(p)) return true; }
  if (!o[0]) return false; // reached the end of the prototype chain
  if (seenObjs.member(function(o2) { return o === o2; }))
    return false;
  else
    seenObjs.push(o);
  var found = false, slen = seenObjs.length;
  o[0].forEachObj(function(proto) {
      if (found) return; //forEachObj keeps looping even tho we have the answer.
      if (hasProp(proto, pred, seenObjs)) found = true;
      seenObjs.splice(slen, seenObjs.length);
    });
  return found;
};
...
function makeDefaultProto(n) {
  ...
  o["constructor-"] = new Aprop(new Aval(NOBASEVALS, n.addr));
