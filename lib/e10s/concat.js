...
function concat(root, jses, jsms, xuls, jsOut, coordsOut) {
    var currentLine = 1;
    var coords = {};

    var jsStream = fs.createWriteStream(jsOut, { encoding: 'utf8' });
    ...
    function puts(str, cb) {
        if (jsStream.write(str))
            process.nextTick(cb);
        else
            jsStream.once("drain", cb);
    }
    ...
    function cat(file, cb) {
        ...
        stream.pipe(jsStream, { end: false });
    ...
    function print(file) {
        var munged = file.replace(/[.\/]/g, "_").replace(/[^a-z0-9_$]*/g, "");
        return function printScript(script, cb) {
            if (script.contents) {
                var contents = script.eventType
                    ? "function $handler_" + munged + "_" + script.line + "(event) { " + script.contents + "}"
                    : script.contents;
                contents = contents + "\n";
                if (!coords[file])
                    coords[file] = {};
                coords[file][script.line] = currentLine;
                countLines(null, contents);
                puts(contents, cb);
            } else {
                process.nextTick(cb);
            }
...
function main(args) {
    ...
    var js = args[4];
    var coords = args[5];
    concat(root, jses, jsms, xuls, js, coords);
}
