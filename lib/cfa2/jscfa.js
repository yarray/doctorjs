...
// function node, array of Aval -> {args, timestamp, boolean} or false
function searchPending(n, args) {
  var bucket = pending[n.addr];
...
function rmPending(n, elm) { return pending[n.addr].pop(); }
...
var dbg_stack = [], dbg_depth = 1000, printedonce = false;
...
function evalFun(fn, args, withNew, cn) {
  ...
  var pmsg = fn.fname.name + " " + timestamp + ", " + (args[1] && args[1].base);
  dbg_stack.push(pmsg);

  // if (dbg_stack.length >= dbg_depth) {
  //   if (dbg_stack.length >= 11000  && !printedonce) {
  //     printedonce = true;
  //     var fd=fs.openSync("/home/dimvar/work-wcopy/e10s-project/addons/dbgstack"+
  //                        11000 + ".txt", "w", mode=0777);
  //     printf(fd, dbg_stack.join("\n"));
  //     // for (var j = 1, slen = dbg_stack.length; j < 500; j++)
  //     //   printf(fd, dbg_stack[slen - i] + "\n");
  //     fs.closeSync(fd);
  //   }

  //   print("big stack " + dbg_depth);
  //   dbg_depth += 2000;
  // }

  function dbg_return(v) {
    dbg_stack.pop();
    return v;
  }
  ...
  if (fn.builtin) return dbg_return(fn.body(args, withNew, cn));

  ++callsEvalFun; // don't count the built-ins

  var tsAtStart;
  var result = searchSummary(fn, args, timestamp);
  if (result) return dbg_return(new Ans(result[0], undefined, result[1]));
  ...
  while(true) {
    try {
      ...
      var pelm2 = searchPending(fn, args), p2ts = pelm2 && pelm2.ts;
      if (!p2ts) {
        ...
        if (pelm2.recurflag) {
          return dbg_return(new Ans(BOTTOM));
        }
        else {
      ...
      return dbg_return(new Ans(retval, undefined, errval));
    }
    catch (e) {
      if (!pelm1) { 
        dbg_stack.pop();
      ...
      if (e.fn === fn && arrayeq(aveq, e.args, args)) {
        // There may be two elms in pending that match the error, remove both.
        if (rmPending(fn, pelm1).recurflag) {
          dbg_stack.pop();
          throw e;
        }
      }
