...
const BINARY = [BITWISE_OR, BITWISE_XOR, BITWISE_AND, EQ, NE, STRICT_EQ, 
                ...
                MINUS, MUL, DIV, MOD, DOT, AND, OR, ASSIGN, INDEX, IN, DOT, 
                PLUS_ASSIGN];
...
function fixExp(n, funName) {
  ...
  if (nt === HOOK || N_ARY.memq(nt)) {
    ch.forEach(fixe);
    return n;
  }

  if (nt === CALL || nt === NEW_WITH_ARGS) {
    ch[0] = fixExp(ch[0]);
...
function walkASTgenerator() {
  ...
  jt[CALL] = jt[NEW_WITH_ARGS] = _call;
  jt[OBJECT_INIT] = _obj_init;
...
(function() {
  ...
  override(REGEXP, function(n) { n.addr = ++count; });
...
(function() {
  ...
  override(IDENTIFIER, function(n, innerscope, otherscopes) {
  ...
  override(OBJECT_INIT, function(n, innerscope, otherscopes) {
    // don't classify property names
    n.children.forEach(function(prop) {
      if (prop.type !== GETTER && prop.type !== SETTER)
        tagVarRefs(prop.children[1], innerscope, otherscopes);
    });
  });
...
(function() {
  ...
  override(OBJECT_INIT, function(n) {
    n.children.forEach(function(p) {
      if (p.type !== GETTER && p.type !== SETTER) markConts(p.children[1]);
    });
  });
...
(function() {
  ...
  override(HOOK, function(n, fr) {
    var ch = n.children;
    var nt = n.type, ans = evalExp(ch[0], fr);
    var ans1 = evalExp(ch[1], ans.fr);
    var ans2 = evalExp(ch[2], ans1.fr);
  ...
  override(OBJECT_INIT, function(n, fr) {
    ...
    n.children.forEach(function(pinit) {
      ...
      fr = ans.fr;
      newobj.updateProp(pinit.children[0].name, ans.v);
      errval = maybeavjoin(errval, ans.err);
    });
  ...
  override(DOT, function(n, fr) {
    ...
    ans.v = avobj.getProp(n.children[1].name);
    return ans;
  });
...
(function() {
  ...
  tagVarRefs_override(IDENTIFIER, function(n, innerscope, otherscopes) {
    var boundvar, varname = n.name;
    // search var in innermost scope
    for (var i = innerscope.length - 1; i >= 0; i--) {
      boundvar = innerscope[i];
