...
opArity[NEW] = opArity[INCREMENT] = opArity[DECREMENT] = UNARY;
...
function fixExp(n, funName) {
  ...
  case BINARY:
    if (nt === INDEX) {
      ...
      if (ch[1].value === "prototype") n.type = DOT_PROTO;
      ch[1].value += "-";
    }
  ...
  case MULTI_OI:
    ch.forEach(function(prop) {
      ...
      var pch = prop.children, pname = pch[0].value;
      pch[0] = idNode(pch[0].tokenizer, pname + "-");
...
function fixStm(n) {
  var i, j, n2, n3, ans1, ans2, ch = n.children;
  ...
  function initsToAssigns(n) {
    var n2, vdecl, a, initv, i, len, ch = n.children, ch2;
    
    n2 = new Node(n.tokenizer, {type:COMMA});
    ch2 = n2.children;
    for (i=0, len=ch.length; i < len; i++) {
      ...
      if (initv) {
        ...
        a.children.push(idNode(vdecl.tokenizer, vdecl.name));
        a.children.push(initv);
...
function labelExp(n) {
  ...
  case MULTI_CALL:
    ...
    labelExp(ch[0]);
    ch[1].children.forEach(labelExp);
    return;
  ...
  case MULTI_OI:
    ...
    ch.forEach(function(prop) {
      ...
      labelExp(prop.children[0]); labelExp(prop.children[1]);
    });
    return;
  }
}
...
(function() {
  ...
  function _binary(n, innerscope, otherscopes) {
    ...
    if (n.type === DOT) {// don't classify property names
      ...
      if (commonJSmode && n0.type === IDENTIFIER && n0.name === "exports")
        tagVarRefsExp[opArity[n0.type]](n0, innerscope, otherscopes, ch[1]);
      else 
        tagVarRefsExp[opArity[n0.type]](n0, innerscope, otherscopes);
      return;
    }
    else if (n.type === INDEX) {
      var n0 = ch[0], shadowed = false;
  ...
  function _recurTag(n, innerscope, otherscopes) {
    n.children.forEach(function(rand) { 
  ...
  function _call(n, innerscope, otherscopes) {
    var ch = n.children;
    tagVarRefsExp[opArity[ch[0].type]](ch[0], innerscope, otherscopes);
    ch[1].children.forEach(function(arg) {
  ...
  function _nullary(n, innerscope, otherscopes) {
    ...
    n.type = DOT;
    var nthis = idNode({}, "jscfaInternalVar: global object");
    nthis.kind = HEAP;
    nthis.addr = global_object_av_addr;
    n.children.push(nthis);
    n.children.push(idNode({}, n.name + "-"));
  }

  function _obj_init(n, innerscope, otherscopes) {
    // don't classify property names
    n.children.forEach(function(prop) {
      var pch = prop.children;
      if (prop.type !== GETTER && prop.type !== SETTER)
        tagVarRefsExp[opArity[pch[1].type]](pch[1], innerscope, otherscopes);
    });
  }
...
function markConts(n, kreg, kexc) {
  ...
  function markContsExp(n) {
    ...
    case UNARY: case BINARY: case TERNARY: case MULTI:
      n.children.forEach(markContsExp);
      return;

    case MULTI_CALL:
      markContsExp(ch[0]);
      ch[1].children.forEach(markContsExp);
      return;
...
Aval.prototype.callFun = function(args, callNode) {
  ...
  //     break;
  //   case DOT:
  //     if (ratorNode.children[1].type === IDENTIFIER) {
  //       funName = ratorNode.children[1].name.slice(0, -1);
  //       break;
  //     }
  //     // fall thru
  //   default:
...
// function node, [args, timestamp, depth, boolean] -> void
...
(function() {
  ...
  function _multi(n) {
    ...
    n.children.forEach(function(e) { initDeclsExp[opArity[e.type]](e); });
  }
  ...
  function _call(n) {
    ...
    initDeclsExp[opArity[ch[0].type]](ch[0]);
    ch[1].children.forEach(function(e) { initDeclsExp[opArity[e.type]](e); });
  }
  ...
  function _obj_init(n) {
    ...
    n.children.forEach(function(prop) {
      var pch = prop.children;
      if (prop.type !== GETTER && prop.type !== SETTER) {
        initDeclsExp[opArity[pch[0].type]](pch[0]);
        initDeclsExp[opArity[pch[1].type]](pch[1]);
      }
    });
  }
...
(function() {
  ...
  function _script(n) {
    ...
    n.children.forEach(function(s) { initDeclsInHeap[s.type](s); });
  }
...
(function() {
  ...
  function _index(n, ans, oldlval) {
    ...
    var prop = ch[1], pt = prop.type;
    var ansobj = evalExp[ch[0].type](ch[0], fr), avobj = ansobj.v;
    fr = ansobj.fr;
    errval = ansobj.err;
  ...
  function _dot(n, ans, oldlval) {
    ...
    var ch = n.children, ans2 = evalExp[ch[0].type](ch[0], ans.fr);
    ans2.v.updateProp(ch[1].name, ans.v);
    ans.fr = ans2.fr;
...
(function() {
  ...
  function _true(n, fr) { return new Ans(ATRU, fr); }

  function _false(n, fr) { return new Ans(AFALS, fr); }
  ...
  function _unary2num(n, fr) {
    var n0 = n.children[0], ans = evalExp[n0.type](n0, fr);
    ans.v = ANUM;
    return ans;
  }

  function _not(n, fr) {
    var n0 = n.children[0], ans = evalExp[n0.type](n0, fr), av = ans.v;
    if (av.isTruthy())
      ans.v = AFALS;
    else if (av.isFalsy())
      ans.v = ATRU;
  ...
  function _in(n, fr) {
    var ch = n.children;
    var ans1 = evalExp[ch[0].type](ch[0], fr);
    var ans2 = evalExp[ch[1].type](ch[1], ans1.fr);
  ...
  function _andor(pred1, pred2) {
    return function(n, fr) {
      var ch = n.children;
      var ans1 = evalExp[ch[0].type](ch[0], fr), av = ans1.v;
  ...
  function _plus_assign(n, fr) {
    ...
    var ans = evalExp[ch[0].type](ch[0], fr);
    return evalLval[ch[0].type](ch[0], evalExp[ch[1].type](ch[1], fr), ans.v);
  }

  function _assign(n, fr) {
    var ch = n.children;
    return evalLval[ch[0].type](ch[0], evalExp[ch[1].type](ch[1], fr));
  }

  function _hook(n, fr) {
    var ch = n.children;
    var nt = n.type, ans = evalExp[ch[0].type](ch[0], fr);
    var ans1 = evalExp[ch[1].type](ch[1], ans.fr);
    var ans2 = evalExp[ch[2].type](ch[2], ans1.fr);
  ...
  function _obj_init(n, fr) {
    ...
    n.children.forEach(function(pinit) {
      if (pinit.type === GETTER || pinit.type === SETTER)
        return;
      var pch = pinit.children;
      ans = evalExp[pch[1].type](pch[1], fr);
      fr = ans.fr;
      newobj.updateProp(pch[0].name, ans.v);
      errval = maybeavjoin(errval, ans.err);
    });
  ...
  function _dot_proto(n, fr) {
    var n0 = n.children[0], ans = evalExp[n0.type](n0, fr), ans2, av = BOTTOM, 
    av2, errval = ans.err;
  ...
  function _index(n, fr) {
    var ch = n.children;
    var ansobj = evalExp[ch[0].type](ch[0], fr), avobj = ansobj.v.baseToObj(),
    prop = ch[1], pt = prop.type, errval = ansobj.err, av , ans;
    fr = ansobj.fr;
  ...
  function _dot(n, fr) {
    var ch = n.children;
    var ans = evalExp[ch[0].type](ch[0], fr), avobj = ans.v.baseToObj();
    ans.thisav = avobj; // used by method calls
    ans.v = avobj.getProp(ch[1].name);
    return ans;
  }

  function _call(n, fr) {
    var ch = n.children;
    var ans = evalExp[ch[0].type](ch[0], fr), ans1, errval, rands = [];
  ...
  function _new_with_args(n, fr) {
    ...
    var ans = evalExp[ch[0].type](ch[0], fr), ans1, errval;
...
(function() {
  ...
  function _ide_bin(n) {
    var ch = n.children;
    if (n.type === DOT && ch[1].value === "addEventListener-") {
      n.addr = ++count;
      ++eventClassif.total;
    ...
    ch.forEach(function(e) { initDeclsExp[opArity[e.type]](e); });
  }
...
function initDOMObjs() {
  ...
  function aEL(args, withNew, callNode) {
    ...
    var ratorNode = callNode.children[0], ec, kind, r;
